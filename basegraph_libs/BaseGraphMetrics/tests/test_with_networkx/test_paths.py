...
def test_shortest_paths(small_directed_fixture, small_undirected_fixture):
    for bg_graph, nx_graph, vertices in small_directed_fixture+small_undirected_fixture:
        ...
        for i, vertex in enumerate(vertices):
            bg_path1 = [[vertices[k] for k in path] for path in core.algorithms.find_geodesics_from_vertex(bg_graph, i)]
            for j in bg_graph:
                if not (i<=j or nx_graph.is_directed()):  # Checking only i<=j for undirected graphs
                   break

                bg_path2 = [vertices[k] for k in core.algorithms.find_geodesics(bg_graph, i, j)]
...
def test_all_shortest_paths(small_directed_fixture, small_undirected_fixture):
    for bg_graph, nx_graph, vertices in small_directed_fixture+small_undirected_fixture:
        for i, vertex in enumerate(vertices):
            bg_path1 = [[[vertices[k] for k in path] for path in paths] for paths in core.algorithms.find_all_geodesics_from_vertex(bg_graph, i)]
            for j in bg_graph:
                if not (i<=j or nx_graph.is_directed()):  # Checking only i<=j for undirected graphs
                   break

                bg_path2 = [[vertices[k] for k in path] for path in core.algorithms.find_all_geodesics(bg_graph, i, j)]
